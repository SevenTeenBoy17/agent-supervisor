...
trap {
    [Console]::Error.WriteLine('{"status":"degraded","reason":"finalize-adapter-failure"}')
    exit 4
}
...
$argsList = @('finalize', '--runtime', 'codex', '--workspace', $workspacePath, '--session', $SessionId, '--round', $RoundId)
if ($null -ne $StopAttempt) { $argsList += @('--stop-attempt', [string]$StopAttempt) }
if ($Blocked) { $argsList += '--blocked' }
if ($projectResolution.Present) { $argsList += @('--project-file', $ProjectFile) }
$invocation = Invoke-AgentSupervisorPython -Command $pythonCommand -PrefixArgs $pythonPrefix -Arguments (@('-E', '-P', '-X', 'utf8', $launcherPath) + $argsList) -Operation 'finalize' -WorkingDirectory $coreRoot -IsolatedEnvironment
