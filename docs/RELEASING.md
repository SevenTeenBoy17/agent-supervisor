...
5. Build from a clean checkout with line-ending conversion disabled. The identity JSON
   is local validation data and must not be uploaded. Before invoking the builder,
   register the machine's canonical Git executable and its SHA-256 as the `git` entry
   in the machine-local `TrustedExecutableRegistry/v1`; the builder ignores ambient
   `PATH` resolution and fails closed if that entry is absent or has drifted. The
   `--version` value must exactly equal the checkout's `VERSION` file:
   ...
   python bin/build-core-release-manifest.py --root . --version 3.1.12 --output .ci-artifacts/agent-supervisor-3.1.12.zip --identity-output .ci-artifacts/release-identity.json
   ...
   `agent-supervisor-3.1.12.zip` and
   `agent_supervisor_core-3.1.12-py3-none-any.whl`.
